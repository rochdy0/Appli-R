...
class VoiApi {
  ...
  VoiApi({http.Client? client}) : client = client ?? http.Client();
  ...
  Future<Set<VoiVehiculeModel>> fetchVehicle() async {
    ...
    final response = await client.get(url, headers: {'origin': 'test_mon_appli'});
    ...
    return bikes
        .map((json) => VoiVehiculeModel.fromJson(json))
        .toSet();
  }
}
